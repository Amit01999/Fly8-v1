...
const StudentDashboardHome = () => {
  ...
  const applications: any[] = []; // no applications yet
  const scholarships: any[] = []; // no scholarships yet
  const communities: any[] = []; // no communities yet
          ...
          <CardContent className="space-y-3">
            {applications.length === 0 ? (
              <p className="text-muted-foreground text-sm">
                No applications yet. Start by exploring universities!
              </p>
            ) : (
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {scholarships.length === 0 ? (
              <p className="text-muted-foreground text-sm">
                No scholarships available yet. Complete your profile to see
                matches.
              </p>
            ) : (
              scholarships.map(sch => (
                <div key={sch.id} className="p-3 border rounded-lg">
                  {/* sch info here */}
                </div>
              ))
            )}
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {communities.length === 0 ? (
              <p className="text-muted-foreground text-sm">
                No community groups joined yet.
              </p>
            ) : (
              communities.map(com => (
                <div key={com.id} className="p-3 border rounded-lg">
                  {/* community info */}
                </div>
              ))
            )}
